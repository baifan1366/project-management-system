    ...
    agile: {
      getTeamAgileByTeamId: async (teamId) => {
        try {
          ...
          console.log('【API调用】getTeamAgileByTeamId, teamId:', teamId);
          const res = await fetchWithTokenRefresh(`${API_BASE}/teams/agile?teamId=${teamId}&type=agile`);
          
          if (!res.ok) {
            const errorData = await res.json();
            console.error('获取团队敏捷数据失败:', errorData);
            throw new Error(errorData.error || '获取团队敏捷数据失败');
          }
          
          const data = await res.json();
          console.log('【API响应】getTeamAgileByTeamId:', data);
          return { data: Array.isArray(data) ? data : [] };
        } catch (error) {
          console.error('获取团队敏捷数据异常:', error);
          throw error;
        }
      },
      getTeamAgileById: async (agileId) => {
        try {
          if (!agileId) {
            throw new Error('敏捷ID不能为空');
          }
          
          console.log('【API调用】getTeamAgileById, agileId:', agileId);
          const res = await fetchWithTokenRefresh(`${API_BASE}/teams/agile/detail?agileId=${agileId}`);
          
          if (!res.ok) {
            const errorData = await res.json();
            console.error('获取敏捷详情失败:', errorData);
            throw new Error(errorData.error || '获取敏捷详情失败');
          }
          
          const data = await res.json();
          console.log('【API响应】getTeamAgileById:', data);
          return { data };
        } catch (error) {
          console.error('获取敏捷详情异常:', error);
          throw error;
        }
      },
      getAgileRolesByTeamId: async (teamId) => {
        try {
          ...
          console.log('【API调用】getAgileRolesByTeamId, teamId:', teamId);
          const res = await fetchWithTokenRefresh(`${API_BASE}/teams/agile?teamId=${teamId}&type=roles`);
          
          if (!res.ok) {
            const errorData = await res.json();
            console.error('获取敏捷角色失败:', errorData);
            throw new Error(errorData.error || '获取敏捷角色失败');
          }
          
          const data = await res.json();
          console.log('【API响应】getAgileRolesByTeamId:', data);
          return { data: Array.isArray(data) ? data : [] };
        } catch (error) {
          console.error('获取敏捷角色异常:', error);
          throw error;
        }
      },
      getAgileMembersByAgileId: async (agileId) => {
        try {
          ...
          console.log('【API调用】getAgileMembersByAgileId, agileId:', agileId);
          const res = await fetchWithTokenRefresh(`${API_BASE}/teams/agile?agileId=${agileId}`);
          
          if (!res.ok) {
            const errorData = await res.json();
            console.error('获取敏捷成员失败:', errorData);
            throw new Error(errorData.error || '获取敏捷成员失败');
          }
          
          const data = await res.json();
          console.log('【API响应】getAgileMembersByAgileId:', data);
          return { data: Array.isArray(data) ? data : [] };
        } catch (error) {
          console.error('获取敏捷成员异常:', error);
          throw error;
        }
      },
      getAgileRoleById: async (roleId) => {
        try {
          ...
          console.log('【API调用】getAgileRoleById, roleId:', roleId);
          const res = await fetchWithTokenRefresh(`${API_BASE}/teams/agile?roleId=${roleId}`);
          
          if (!res.ok) {
            const errorData = await res.json();
            console.error('获取角色信息失败:', errorData);
            throw new Error(errorData.error || '获取角色信息失败');
          }
          
          const data = await res.json();
          console.log('【API响应】getAgileRoleById:', data);
          return { data };
        } catch (error) {
