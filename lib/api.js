  ...
  teams: {
    ...
    teamSectionTasks: {
      listBySectionId: async (sectionId) => {
        ...
        if (!res.ok) throw new Error('Failed to fetch team section tasks')
        return res.json()
      },
      listByUserId: async (userId) => {
        if (!userId || userId === 'current') {
          throw new Error('必须提供有效的用户ID，不能使用"current"标识符');
        }
        
        const res = await fetch(`${API_BASE}/teams/section/tasks/user?userId=${userId}`)
        if (!res.ok) throw new Error('Failed to fetch user tasks')
        return res.json()
      },
      listAllTasks: async () => {
        const res = await fetch(`${API_BASE}/teams/section/tasks/all`)
        if (!res.ok) throw new Error('Failed to fetch all tasks')
        return res.json()
      },
      getById: async (sectionId, taskId) => {
        const res = await fetch(`${API_BASE}/teams/section/tasks?sectionId=${sectionId}&taskId=${taskId}`)
        if (!res.ok) throw new Error('Failed to fetch team section task details')
        return res.json()
      },
      create: async (taskData) => {
        ...
        if (!res.ok) throw new Error('Failed to create team section task')
        return res.json()
      },
      update: async (sectionId, taskId, taskData) => {
        const res = await fetch(`${API_BASE}/teams/section/tasks?sectionId=${sectionId}&taskId=${taskId}`, {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
        ...
        if (!res.ok) throw new Error('Failed to update team section task')
        return res.json()
      },
      updateDirectly: async (taskId, taskData) => {
        ...
        if (!res.ok) throw new Error('Failed to update task directly')
        return res.json()
      },
      updateTaskIds: async (sectionId, teamId, taskIds) => {
        const res = await fetch(`${API_BASE}/teams/section?teamId=${teamId}&sectionId=${sectionId}`, {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
      ...
      delete: async (sectionId, taskId) => {
        const res = await fetch(`${API_BASE}/teams/section/tasks?sectionId=${sectionId}&taskId=${taskId}`, {
          method: 'DELETE',
        })
        if (!res.ok) throw new Error('Failed to delete team section task')  
        return res.json()
      },
    }
  },
