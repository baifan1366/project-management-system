...
export const subscriptionTrackingMiddleware = store => next => async action => {
  ...
  // When the current user is fetched successfully, also fetch their subscription data.
  if (action.type === 'users/fetchCurrentUser/fulfilled') {
    const userId = action.payload?.id;
    if (userId) {
      // Dispatch the fetchSubscription thunk to keep subscription data in sync.
      store.dispatch(fetchSubscription(userId));
    }
  }
  ...
  if (action.type.endsWith('/fulfilled')) {
    ...
    // Skip query operations, notifications, and createTeam (handled in UI)
    if (
      actionType === 'createTeam' ||
      actionType.startsWith('fetch') ||
