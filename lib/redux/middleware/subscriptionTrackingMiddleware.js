...
import { limitExceeded } from '../features/subscriptionSlice';
...
export const subscriptionTrackingMiddleware = store => next => async action => {
  ...
  if (action.type === 'projects/createProject/pending' || 
      action.type === 'teamMembers/inviteMember/pending' ||
      action.type === 'teams/createTeam/pending') {
    ...
    } else if (action.type === 'teamMembers/inviteMember/pending') {
      limitActionType = 'invite_member';
      userId = action.meta?.arg?.inviter_id || action.meta?.arg?.user_id;
    } else if (action.type === 'teams/createTeam/pending') {
