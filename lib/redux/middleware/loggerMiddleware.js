...
export const loggerMiddleware = store => next => action => {
  ...
  if (action.type.endsWith('/fulfilled')) {
    ...
    if ((actionType.startsWith('fetch')) || 
        ...
        actionType === 'updateOrder' || actionType === 'updateTeamCustomFieldOrder' ||
    ...
    console.log('Debug - Auth State:', store.getState().auth); // 调试信息
    console.log('Debug - Action:', action); // 调试信息

    let userId = null;
    
    // 特殊处理 teamUserInv 相关操作
    if (action.type === 'teamUserInv/create/fulfilled' || action.type === 'teamUserInv/updateStatus/fulfilled') {
      // 从 action.meta.arg 中获取创建邀请的用户信息
      const currentUser = store.getState().auth?.user;
      userId = currentUser?.id || action.meta?.arg?.created_by;
      
      if (!userId) {
        // 如果还是获取不到，尝试从 localStorage 获取
        const session = JSON.parse(localStorage.getItem('supabase.auth.token'));
        userId = session?.user?.id;
      }
      
      console.log('Debug - TeamUserInv Create - UserId:', userId);
    } else {
    ...
    if (!userId) {
      console.warn('No user ID found for logging:', action, 'Auth State:', store.getState().auth)
      return result
    }

    // 为createTeam、createTeamUser和邀请相关操作添加特殊的日志记录
    if (action.type === 'teams/createTeam/fulfilled' || 
        action.type === 'teams/createTeamUser/fulfilled' ||
        action.type === 'teamUserInv/updateStatus/fulfilled' ||
        action.type === 'teamUserInv/create/fulfilled') {
      ...
      const specialLogData = {
        ...
        details: action.type === 'teams/createTeam/fulfilled'
          ...
          : action.type === 'teamUserInv/updateStatus/fulfilled'
          ? { invitationId: action.payload.id, status: action.payload.status }
          : action.type === 'teamUserInv/create/fulfilled'
          ? { teamId: action.payload.team_id, userEmail: action.payload.user_email, role: action.payload.role }
          : action.type === 'teamUsersInv/fetchTeamUsersInv/fulfilled'
