...
export const fetchCurrentUserPlan = createAsyncThunk(
  ...
  async (userData, { rejectWithValue }) => {
    try {
      ...
      // Query user's active subscription plan only
      const { data, error } = await supabase
        ...
        .eq('status', 'ACTIVE') // Only fetch active plans
        .order('start_date', { ascending: false }) // Get the most recent active plan
        .limit(1) // Ensure we only get one record
        .single();
      
      if (error) {
        // If no active plan found, return null plan
