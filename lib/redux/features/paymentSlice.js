  ...
  async (paymentIntentId, { rejectWithValue }) => {
    try {
      ...
      if (!paymentIntentId || typeof paymentIntentId !== 'string') {
        ...
        return rejectWithValue('Invalid payment intent ID format');
      }
      ...
      if (!response.ok) {
        const errorText = await response.text();
        return rejectWithValue(`Failed to fetch payment status: ${errorText}`);
      }
      ...
      return rejectWithValue(error.message || 'Unknown payment status error');
    }
  }
)
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchPaymentStatus.rejected, (state, action) => {
        state.status = 'failed';
        state.error = action.error.message || action.payload || 'Failed to fetch payment status';
      })
