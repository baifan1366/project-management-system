...
export const loginAdmin = createAsyncThunk(
  ...
  async ({ email, password }, { rejectWithValue }) => {
    try {
      ...
      let permissions = [];
      try {
        // Get permission IDs for the admin role
        const { data: rolePermissions, error: permissionsError } = await supabase
          .from('admin_role_permission')
          .select('permission_id')
          .eq('role', adminDetails.role);
        ...
        if (!permissionsError && rolePermissions && rolePermissions.length > 0) {
          ...
          console.log('No role permissions found for role:', adminDetails.role);
        }
      } catch (permError) {
...
export const checkAdminPermissions = createAsyncThunk(
  ...
  async (role, { rejectWithValue, getState }) => {
    try {
      if (!role) {
        // If no role provided, try to get it from state
        const state = getState();
        role = state.admin?.admin?.role;
        
        if (!role) {
          throw new Error('No admin role provided or available in state');
        }
      }
