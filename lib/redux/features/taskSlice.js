...
export const deleteTask = createAsyncThunk(
  ...
  async ({ sectionId, userId, oldValues, taskId, teamId }, { dispatch, rejectWithValue }) => {
    try {
      ...
      //from the mytask record, get the user_id array
      ...
      //create an alert notification for every related assignee
      for (const assigneeId of assigneeIds) {
      ...
      try {
        ...
        if (deleteMyTasksError) {
          console.error('Error deleting associated mytasks records:', deleteMyTasksError);
        }
        
      } catch (mytasksError) {
        console.error('Failed to delete associated mytasks:', mytasksError);
      }
