...
export const fetchTeamUserRole = createAsyncThunk(
  'teamUsers/fetchTeamUserRole',
  async (teamId, { rejectWithValue }) => {
    try {
      const res = await api.teams.getTeamUserRole(teamId);
      return { teamId, role: res };
    } catch (error) {
      console.error('Redux: 获取团队用户角色失败:', error);
      return rejectWithValue(error.message || '获取团队用户角色失败');
    }
  }
);
