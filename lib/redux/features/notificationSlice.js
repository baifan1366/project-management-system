...
export const fetchNotifications = createAsyncThunk(
  ...
  async (userId, { rejectWithValue, dispatch, getState }) => {
    try {
      ...
      // Modified debounce logic - if called from a UI component refresh, 
      // force a refresh regardless of debounce
      const now = Date.now();
      const forceRefresh = !!getState().notifications?.forceRefresh;
      
      // Only apply debounce if not forcing refresh
      if (!forceRefresh && lastFetchTime && lastFetchUserId === userId && 
          now - lastFetchTime < FETCH_DEBOUNCE_MS) {
...
const notificationSlice = createSlice({
  ...
  initialState: {
    ...
    forceRefresh: false, // Flag to force refresh ignoring debounce
  },
  reducers: {
    ...
    // Force refresh flag for next fetch
    ...
    builder
      ...
      .addCase(fetchNotifications.fulfilled, (state, action) => {
        ...
        if (Array.isArray(action.payload)) {
          ...
          // First ensure we have the correct userId from the payload
          if (action.meta?.arg) {
            state.userId = action.meta.arg; // Set userId from the original action argument
          }
          
          // Now calculate unreadCount with the confirmed userId
          state.unreadCount = action.payload.filter(
            notification => !notification.is_read && 
            // If we have a userId, filter by it, otherwise count all unread
            (state.userId ? notification.user_id === state.userId : true)
          ).length;
