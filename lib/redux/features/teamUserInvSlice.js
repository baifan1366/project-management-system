...
export const createTeamUserInv = createAsyncThunk(
  'teamUserInv/create',
  async ({ teamId, userEmail, role, created_by }, { getState, rejectWithValue, dispatch }) => {
    try {      
      if (!created_by) {
      ...
      //get user.id from user table using userEmail
      ...
      // Send notification to the invited user if we have their user ID
      if (response && userId) {
