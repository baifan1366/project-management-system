...
export const getTags = createAsyncThunk(
  ...
  async (teamId, teamCFId, { rejectWithValue }) => {
    try {
      const response = await api.teamCustomFields.getTags(teamId, teamCFId);
      return response; // 返回获取的标签数据
    } catch (error) {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getTags.fulfilled, (state, action) => {
        state.status = 'succeeded'
        state.loading = false; // 请求成功后设置 loading 为 false
        state.tags = action.payload; // 保存获取的标签数据
      })
      .addCase(getTags.rejected, (state, action) => {
        state.status = 'failed'
        state.loading = false; // 请求失败后设置 loading 为 false
