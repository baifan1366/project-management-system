...
export const getTagByName = createAsyncThunk(
    'tags/getTagByName',
    async (tagName) => {
        const res = await api.tags.getByName(tagName)
        return res.id;  //return tag id
    }
)
...
const tagSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(getTagByName.pending, (state) => {
                state.status = 'loading'
            })
            .addCase(getTagByName.fulfilled, (state, action) => {
                state.status = 'succeeded'
                state.currentTag = action.payload
            })
            .addCase(getTagByName.rejected, (state, action) => {
                state.status = 'failed'
                state.error = action.error.message
            })
